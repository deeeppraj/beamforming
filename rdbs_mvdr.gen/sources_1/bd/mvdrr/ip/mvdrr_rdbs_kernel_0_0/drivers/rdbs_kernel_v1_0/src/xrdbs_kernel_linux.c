...
typedef struct {
    ...
    XRdbs_kernel_uio_map maps[ MAX_UIO_MAPS ];
} XRdbs_kernel_uio_info;

/***************** Variable Definitions **************************************/
static XRdbs_kernel_uio_info uio_info;
...
static int uio_info_read_name(XRdbs_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRdbs_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRdbs_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRdbs_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRdbs_kernel_Initialize(XRdbs_kernel *InstancePtr, const char* InstanceName) {
	XRdbs_kernel_uio_info *InfoPtr = &uio_info;
