...
typedef struct {
    ...
    XMvdr_weights_uio_map maps[ MAX_UIO_MAPS ];
} XMvdr_weights_uio_info;

/***************** Variable Definitions **************************************/
static XMvdr_weights_uio_info uio_info;
...
static int uio_info_read_name(XMvdr_weights_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMvdr_weights_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMvdr_weights_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMvdr_weights_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMvdr_weights_Initialize(XMvdr_weights *InstancePtr, const char* InstanceName) {
	XMvdr_weights_uio_info *InfoPtr = &uio_info;
