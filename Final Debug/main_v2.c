...
#include "input_samples.h"      /* uint64_t input_samples[64000], one 64-bit
                                    word per sample: bits[31:0]=packed real/imag,
                                    bits[63:32]=0 (RDBS ignores upper 32 bits on x_in) */
...
#define INPUT_SIZE_BYTES (N_CHANNELS * N_SNAPSHOTS * 8)   /* 64-bit words: rdbs_kernel x_in
                                                                synthesized as TDATA[63:0],
                                                                confirmed via component.xml */
...
int main(void)
{
     ...
     * Buffer Length Register is 23 bits (max 8 MB) so the full
     * 512000-byte transfer (64000 samples x 8 bytes, 64-bit TDATA)
     * fits in one shot, giving RDBS a continuous uninterrupted stream. */
